...
import (
	...
	appresolve "github.com/r2d2-ai/ai-box/core/app/resolve"
	...
	for id, act := range a.actions {
		if m, ok := act.(managed.Managed); ok {
			...
			if err != nil {
				...
				logger.Debugf("Action [%s] failed to start due to error [%s]", id, err.Error())
...
func (a *App) startTrigger(trg *triggerWrapper) (bool, error) {
	...
	if err != nil {
		...
		//logger.Debugf("Trigger [ %s ]: Started", id)
		version := ""
...
func (a *App) Stop() error {
	...
	if delayedStopInterval != "" {
		...
		if err != nil {
			...
			logger.Debugf("Delaying application stop by - %s", delayedStopInterval)
			time.Sleep(duration)
		}
	}
	...
	for id, act := range a.actions {
		if m, ok := act.(managed.Managed); ok {
			...
			if err != nil {
				logger.Debugf("Action [%s] failed to stop due to error [%s]", id, err.Error())
				logger.Debugf("StackTrace: %s", debug.Stack())
			}
		}
	}
